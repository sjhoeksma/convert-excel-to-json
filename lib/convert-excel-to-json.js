...
const excelToJson = (function() {
    ...
    const parseSheet = (sheetData, workbook) => {
        ...
        const headerRows = (sheetData.header && sheetData.header.rows) || (_config.header && _config.header.rows) || 0;
        const headerRowToKeys = (sheetData.header && sheetData.header.rowToKeys) || (_config.header && _config.header.rowToKeys);
        const addRowId = (sheetData.addRowId) || (_config.addRowId);
        ...
        for (let cell in sheet) {
            ...
            // Add the row number as id to the rowData
            if (addRowId) rowData.id = row - headerRows;
